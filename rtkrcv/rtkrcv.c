...
static void prsatellite(vt_t *vt, int nf)
{
    ...
    for (i=0;i<MAXSAT;i++) {
        ...
        {
            vt_printf(vt, "azel lower than zero %f \n", rtk.ssat[i].azel[1]);
            continue;
        }
        satno2id(i+1,id);
...
/* get base info from flash ------------------------------------------------------------*/
static void get_base_info_from_flash(void)
{
#ifdef USE_X100
    opt_t *opt;
    rtk_param rtk_parameter;
    if ((opt=searchopt("ant2-postype",sysopts)))
    {
        if (*(int *)opt->var == 0)
        {
            if (parameter_rtk_read((uint8_t*)&rtk_parameter) == 0)
            {
                printf("Read RTK param: %u, %u, %u \n", rtk_parameter.base_lat, rtk_parameter.base_lon, rtk_parameter.base_alt);  
                if ((opt=searchopt("ant2-pos1",sysopts)))
                {
                    double lat = rtk_parameter.base_lat/1e7;
                    *(double *)opt->var = lat;    
                } 
                if ((opt=searchopt("ant2-pos2",sysopts)))
                {
                    double lon = rtk_parameter.base_lon/1e7;
                    *(double *)opt->var = lon;            
                } 
                if ((opt=searchopt("ant2-pos3",sysopts)))
                {
                    double alt = rtk_parameter.base_alt/1e2;
                    *(double *)opt->var = alt;          
                }
            }
        }
...
int main(int argc, char **argv)
{
    ...
    if (port) {
        ...
        int counter = 0;
        while (!(con[0]=con_open(0,dev)) && counter<10) {
            fprintf(stderr,"------>waiting console open\n\r");
            counter++;
            sleepms(1000);
        }
        if (counter==10) {
        ...
        fprintf(stderr,"------>console open ok\n\r");
    }
